...
class ProblemState:
    def __init__(self, board: Board, action: str = None, prev_state = None, cost: int = 0):
        self.board = board
        self.cost = cost
        self.action = action
...
def uniform_cost_search(board: Board):
    ...
    while len(priority_queue) > 0:
        ...
        for action in state.board.get_valid_actions():
            ...
            child_state = ProblemState(updated_board, action, prev_state=state, cost=state.cost + 1)
